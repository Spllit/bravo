...
if ( ! function_exists( 'bravo_setup' ) ) :
	...
	function bravo_setup() {
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'bravo' ),
			)
		);
...
function bravo_widgets_init() {
	register_sidebar(
		array(
			...
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'bravo' ),
...
/* finder sidebar
*/
add_action( 'widgets_init', 'register_finder' );
...
/* widget text-us */
add_action( 'widgets_init', 'text_us' );
...
function text_us(){
	register_sidebar( array(
		...
		'description'   => '',
		'class'         => '',
		'before_widget' => '<a class="widget %2$s">',
		'after_widget'  => "</a>\n",
		'before_title'  => '<h2 class="widgettitle">',
		'after_title'   => "</h2>\n",
	) );
...
function bravo_scripts() {
	wp_enqueue_style( 'bravo-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'bravo-style', 'rtl', 'replace' );

	wp_enqueue_script( 'bravo-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
